...
class UCB(Mutator):
    ...
    def select_action(self, banned_actions=[]):
        ...
        for a in range(self.nActions):
            Hoef[a] = self.empiricalMeans[a] + \
                np.sqrt((2.0 * np.log(self.n+1.0))/self.N[a])
        self.lastAction = -1
        maxVal = np.NINF
        for a in range(self.nActions):
    ...
    def reward(self, rew):
        ...
        self.empiricalMeans[self.lastAction] = (
            self.N[self.lastAction] * self.empiricalMeans[self.lastAction] + rewardVal) / (self.N[self.lastAction] + 1)
